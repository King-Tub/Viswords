...
api_origins = ["http://localhost:5000", "http://127.0.0.1:5000", "http://localhost:5500", "http://127.0.0.1:5500", "http://localhost:3000", "http://127.0.0.1:3000"]

# 如果是Vercel环境，添加部署URL
if vercel_url:
    origins.append(f"https://{vercel_url}")
    api_origins.append(f"https://{vercel_url}")
...
# 初始化豆包API客户端
ARK_API_KEY = app.config['ARK_API_KEY']
logger.info(f"Loaded ARK_API_KEY (first 5 chars): {ARK_API_KEY[:5] if ARK_API_KEY else 'None'}")

# 初始化OpenAI客户端（豆包API）
client = OpenAI(
    base_url=app.config['DOUBAO_API_URL'],
    api_key=ARK_API_KEY,
)

# 豆包模型配置
DOUBAO_MODEL = app.config['DOUBAO_MODEL']
